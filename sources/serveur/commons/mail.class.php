...
class Mail
{
    ...
    */
    public function __construct($type, $values)
    {
        ...
        $this->FilePath = sprintf("%s/mails/%s.xml", COMMONS_DIR, $this->Type);
    ...
    */
    public function Send($to = null, $forcedSubject = null)
    {
        ...
        try {
            ...
            if (isset($to)) {
                if (is_array($to) == false) {
                    ...
                    $sent = @mail($to, ((isset($forcedSubject) && $forcedSubject != null && $forcedSubject != '') ? $forcedSubject : $this->Subject), $this->Message, $this->Headers);
                } elseif (is_array($to)) {
                    foreach ($to as $mail) {
                        $this->parseTemplate();
                        $this->generateNewLineTag($mail);
                        Logger::Info("Mail.Send : Send mail to : $mail / subject : $this->Subject");
                        $sent = @mail($mail, ((isset($forcedSubject) && $forcedSubject != null && $forcedSubject != '') ? $forcedSubject : $this->Subject), $this->Message, $this->Headers);
                    }
                } else {
    ...
    */
    private function generateHeader()
    {
        ...
        $this->Headers .= "From: \"MalesherbUnis\" <contact@malesherbunis.fr>\n";
		$this->Headers .= "Bcc: contact@malesherbunis.com\n"; 
        ...
        try {
            ...
            if (isset($this->Values)) {
                for ($i = 0; $i < count($this->Values); $i++) {
                    ...
                    $this->Message .= "Content-Transfer-Encoding: 8bit #NewLine";
                    $this->Message .= "#NewLine" . $this->HtmlMessage . "#NewLine";
                    $this->Message .= "#NewLine--" . $this->Boundary . "-- #NewLine";
                    $this->Message .= "#NewLine--" . $this->Boundary . "-- #NewLine";
                }
            }
            $this->HtmlMessage = utf8_encode($this->HtmlMessage);
